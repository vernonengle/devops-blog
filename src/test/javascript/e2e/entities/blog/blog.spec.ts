...
describe('Blog e2e test', () => {
    ...
    it('should create and save Blogs', async () => {
        ...
        await blogComponentsPage.clickOnCreateButton();
        await promise.all([
            blogUpdatePage.setContentInput('content'),
            blogUpdatePage.setDatePostedInput('01/01/2001' + protractor.Key.TAB + '02:30AM'),
            blogUpdatePage.bloggerSelectLastOption()
        ]);
        expect(await blogUpdatePage.getContentInput()).to.eq('content');
        expect(await blogUpdatePage.getDatePostedInput()).to.contain('2001-01-01T02:30');
